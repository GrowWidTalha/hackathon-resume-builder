...
function initializeForm(): void {
    ...
    if (form) {
        // Name
        (form.querySelector('[name="name"]') as HTMLInputElement).value = resumeData.name || '';

        // Profession
        (form.querySelector('[name="profession"]') as HTMLInputElement).value = resumeData.profession || '';

        // Email
        (form.querySelector('[name="email"]') as HTMLInputElement).value = resumeData.email || '';

        // Phone
        (form.querySelector('[name="phone"]') as HTMLInputElement).value = resumeData.phone || '';

        // Location
        (form.querySelector('[name="location"]') as HTMLInputElement).value = resumeData.location || '';

        // Website
        (form.querySelector('[name="website"]') as HTMLInputElement).value = resumeData.website || '';

        // Summary
        (form.querySelector('[name="summary"]') as HTMLTextAreaElement).value = resumeData.summary || '';

        // Work Experience
        const workExperienceContainer = form.querySelector('#workExperience') as HTMLElement;
        ...
        resumeData.workExperience.forEach((experience, index) => {
            workExperienceContainer.innerHTML += `
                <div class="work-experience-entry">
                    <input type="text" name="workExperience[${index}].title" value="${experience.title}" placeholder="Title">
                    <input type="text" name="workExperience[${index}].company" value="${experience.company}" placeholder="Company">
                    <input type="text" name="workExperience[${index}].date" value="${experience.date}" placeholder="Date">
                    <textarea name="workExperience[${index}].responsibilities" placeholder="Responsibilities">${experience.responsibilities.join('\n')}</textarea>
                </div>
            `;
        });

        // Skills
        const skillsContainer = form.querySelector('#skills') as HTMLElement;
        skillsContainer.innerHTML = ''; // Clear existing entries
        resumeData.skills.forEach((skill, index) => {
            skillsContainer.innerHTML += `
                <div class="skill-entry">
                    <input type="text" name="skills[${index}]" value="${skill}" placeholder="Skill">
                </div>
            `;
        });

        // Picture
        const pictureInput = form.querySelector('[name="picture"]') as HTMLInputElement;
        if (resumeData.picture) {
            pictureInput.value = resumeData.picture; // Handle picture as data URL or path
        }
    }
}